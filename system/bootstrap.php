...
class Bootstrap {
    ...
    function __construct() {
        ...
        // Switch theme with Device
        $_agent = $uo->get_UserAgent();
        if(in_array($_agent['platform'], array('Android', 'iPhone', 'BlackBerry', 'Windows Phone OS', 'Kindle', 'Kindle Fire', 'Playbook'))
                or in_array($_agent['browser'], array('IEMobile'))){
            Registry::$settings['theme_name'] = THEME_NAME;
            Registry::$settings['config']['front_controller'] = 'Mobile';
            
            if (isset($_GET['switch_theme'])) {
                $switch_theme = $_GET['switch_theme'];
                if ($switch_theme == "desktop") {
                    Registry::$settings['theme_name'] = THEME_NAME;
                    Registry::$settings['config']['front_controller'] = "Welcome";
                }
                if(!empty($switch_theme)){
                    $_SESSION['switch_theme'] = $switch_theme;
                }
            } else if(isset($_SESSION['switch_theme'])) {
                if ($_SESSION['switch_theme'] == "desktop") {
                    Registry::$settings['theme_name'] = THEME_NAME;
                    Registry::$settings['config']['front_controller'] = "Welcome";
                }
            }
            ...
            } elseif(isset($_SESSION['switch_theme'])) {
                if ($_SESSION['switch_theme'] == "mobile") {
                    Registry::$settings['theme_name'] = THEME_NAME;
                    Registry::$settings['config']['front_controller'] = 'Mobile';
                }
            }
        ...
        // Check user login
        if (!isset($_SESSION['user_logged_in']) and isset($_COOKIE['user_login'])) {
            $user_logged_in = @unserialize(stripslashes($_COOKIE['user_login']));
...
//            $uo->second = Registry::$settings['statistics']['check_online_second'];
            ...
            if (isset($url[2])) {
                ...
                if (in_array(strtolower($url[0]), array('link', 'post', 'source', 'page', 'category',))) {
