    ...
    if ($("#link").length > 0) {
        $("#link").change(function() {
            if ($(this).next('img').length > 0) {
                $(this).next('img').attr('src', 'http://g.etfv.co/' + $(this).val());
            } else {
                $(this).after('&nbsp;<img src="http://g.etfv.co/' + $(this).val() + '" width="16" height="16" />');
            }
            $.ajax({
                url: dashboard_url + "/link/check_exists_link",
                type: "POST",
                dataType: "json",
                cache: false,
                data: {
                    link: $(this).val(),
                    original_link: $(this).attr('original_link')
                },
                success: function(response, textStatus, XMLHttpRequest) {
                    if (response && response.message != '') {
                        ...
                        elm.after(html);
                        $('body,html').animate({
                            scrollTop: elm.offset().top - elm.outerHeight(true)
                        }, 800);
                    }
                },
                error: function(MLHttpRequest, textStatus, errorThrown) {
