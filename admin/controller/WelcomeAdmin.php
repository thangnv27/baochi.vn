...
//        $row .= "</td></tr>";
        ...
        $row .= "</td></tr>";
        $row .= "<tr><td>RSS " . Language::$phrases['context']['die'] . "</td>";
        $row .= "<td><span class=\"label label-warning\">" . $this->model->countRssDie() . "</span>";
        if ($this->current_user['capability']['rss']['view'] == 1) {
            $row .= " [<a href='" . DASHBOARD_URL . "/rss/?status=1'>" . Language::$phrases['action']['view'] . "</a>]";
        }
        $row .= "</td></tr>";
