...
class LinkCategoryAdmin extends AdminController {

    private $current_user;
    private $taxonomy = 'link';
    ...
    function index() {
        ...
        $url = DASHBOARD_URL . '/linkCategory/';
        switch ($action) {
            case 'delete':
                // Check permission
        ...
        if (is_array($records) and !empty($records)) {
            foreach ($records as $rec) {
                ...
                foreach ($columns as $field => $title) {
                    ...
                    $edit_link = DASHBOARD_URL . '/linkCategory/' . $rec['id'] . '/edit';
                    $delete_link = DASHBOARD_URL . '/linkCategory/' . $rec['id'] . '/delete';
        ...
        if ($request->getMethod() == 'POST') {
            ...
            if ($msg != "") {
                ...
                if ($id) {
                    ...
                    $url = DASHBOARD_URL . '/linkCategory/' . $id . '/edit';
                    $this->redirect($url);
                }
            }
        }
    ...
    function edit($id) {
        ...
        if (count($categories) <= 0) {
            $this->redirect(DASHBOARD_URL . '/linkCategory/');
        } else {
