...
class UserAdmin extends AdminController {
    ...
    public static function checkLogin() {
        if (isset($_SESSION['user_logged_in'])) {
            ...
            $array = array(
                ...
                // Post
                DASHBOARD_URL . '/post/get_from_rss/?req=0/',
                DASHBOARD_URL . '/post/get_from_rss/?req=1/',
                DASHBOARD_URL . '/post/get_from_rss/?req=2/',
                DASHBOARD_URL . '/post/get_from_rss/?req=3/',
                DASHBOARD_URL . '/post/delete_old_posts/',
                // RSS
                DASHBOARD_URL . '/rss/check_rss/',
                // Mobile
                DASHBOARD_URL . '/post/mobile_generate_posts_by_category/?req=0/',
                DASHBOARD_URL . '/post/mobile_generate_posts_by_category/?req=1/',
                DASHBOARD_URL . '/post/mobile_generate_posts_by_category/?req=2/',
                DASHBOARD_URL . '/post/mobile_generate_posts_by_category/?req=3/',
            );
    ...
    function index() {
        ...
        $columns = array(
            ...
            'col_address' => Language::$phrases['context']['address'],
            'col_dob' => Language::$phrases['page']['user']['dob'],
            'col_gender' => Language::$phrases['page']['user']['gender'],
            'col_group' => Language::$phrases['page']['user']['group'],
            'col_registered_date' => Language::$phrases['page']['user']['registered_date'],
            'col_count_link' => Language::$phrases['page']['user']['count_link'],
        ...
        if (is_array($records) and ! empty($records)) {
            foreach ($records as $rec) {
                $usermeta = $this->model->getUserByFieldUnique($rec['id']);
                ...
                foreach ($columns as $field => $col_name) {
                    ...
                    switch ($field) {
                        ...
                        case "col_email":
                            ...
                            break;
                        case "col_address":
                            $row .= '<td ' . $attributes . '>' . $usermeta['address'] . '</td>';
                            break;
                        case "col_dob":
                            $row .= '<td ' . $attributes . '>';
                            if(!empty($usermeta['dob']) and $usermeta['dob'] != '0000-00-00')
                                $row .= date("d-m-Y", strtotime($usermeta['dob']));
                            $row .= '</td>';
                            break;
                        case "col_gender":
                            $row .= '<td ' . $attributes . '>' . Utils::getGender($usermeta['gender']) . '</td>';
                            break;
                        case "col_group":
                        ...
                        case "col_registered_date":
                            ...
                            break;
                        case "col_count_link":
                            $row .= '<td ' . $attributes . '><a href="' . $list_link_user_link . $rec['id'] . '" >' . $this->model->countLinkByUser($rec['id']) . '</a></td>';
                            break;
                        case "col_options":
                            $row .= '<td ' . $attributes . '>';
                            if ($rec['id'] != $this->current_user['id']) {
                                ...
                                if ($rec['groupname'] != 'Banned') {
                                    if ($this->current_user['capability']['users']['edit'] == 1 and $rec['id'] != 1) {
                                        $row .= '<a href="' . $banned_link . '" class="btn btn-warning btn-xs">' . Language::$phrases['action']['banned'] . '</a> ';
                                    }
                                }
    ...
    function banned($user_id) {
        ...
        if (!$user) {
            ...
            $capability = $this->model->getCapabilityByRole('banned');
            $this->model->bannedUser($capability, $user_id);
            $this->getSession()->setFlash('success', Language::$phrases['message']['banned_success']);
        }
        $this->redirect($url);
    }
