...
class CategoryAdmin extends AdminController {
    ...
    function index() {
                ...
                if (count($checked) > 0) {
                    ...
                    if ($cat_ID != "") {
                        ...
                        $this->model->update(array(
                            'parent' => 0,
                                ), "parent IN ($cat_ID) AND lang_code = '" . Language::$lang_content . "' AND taxonomy = 'post'");
                        // Delete
                        $this->model->deleteBulk("id IN ($cat_ID) AND lang_code = '" . Language::$lang_content . "' AND taxonomy = 'post'");
        ...
        $columns = array(
            ...
            'col_id' => 'ID',
            'col_cover' => Language::$phrases['page']['category']['cover'],
        ...
        if (is_array($records) and ! empty($records)) {
            foreach ($records as $rec) {
                ...
                foreach ($columns as $field => $title) {
                    ...
                    $edit_link = DASHBOARD_URL . '/category/' . $rec['id'] . '/edit';
                    $delete_link = DASHBOARD_URL . '/category/' . $rec['id'] . '/delete';
                    ...
                    switch ($field) {
                        ...
                        case "col_cover":
                            $row .= '<td ' . $attributes . '>';
                            if ($rec['cover'] != "") {
                                $row .= '<img src="' . $rec['cover'] . '" width="32" height="32" />';
                            }
                            $row .= '</td>';
                            break;
                        case "col_name":
    ...
    function addnew() {
        ...
        $form->add("name", "text", array(
                ...
                ->add('style', 'choice', array(
                    ...
                    'choices' => Utils::getCategoryStyle(),
                    'data' => $style,
        ));
        ...
        if ($request->getMethod() == 'POST') {
            ...
            if ($msg != "") {
                ...
                if ($id) {
                    ...
                    $url = DASHBOARD_URL . '/category/' . $id . '/edit';
                    $this->redirect($url);
                }
            }
        }
    ...
    function edit($id) {
        ...
        if (count($categories) <= 0) {
            $this->redirect(DASHBOARD_URL . '/category/');
        } else {
