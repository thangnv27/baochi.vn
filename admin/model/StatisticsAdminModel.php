...
class StatisticsAdminModel extends Model {
    ...
    function useronline($where = array()) {
        $count = $this->DB()->select(TABLE_PREFIX . "statistics_useronline", "*", $where);
        return count($count);
    }
    ...
    function visit($time, $daily = null) {
        ...
        if ($daily == true) {
            $result = $this->DB()->get_row(TABLE_PREFIX . "statistics_visit", "*", "last_counter = '{$s->Current_Date('Y-m-d', $time)}'");
            if ($result) {
                return $result['visit'];
            } else {
                return 0;
