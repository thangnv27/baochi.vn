...
class UserAdminModel extends Model {
    ...
    public function countLinkByUser($user_id) {
        $link = $this->DB()->select(TABLE_PREFIX . "links", "COUNT(id) AS total", "user_id = $user_id AND post_status IN ('pending_approval','published') ");
//        array(
...
//        )
    ...
    public function getAllUser() {
        return $this->DB()->select(TABLE_PREFIX . "users", "*", "is_deleted=0");
    }

    public function delete($where) {
        $user_id = $where['id'];
        $db = $this->DB();
        $db->update(TABLE_PREFIX . "users", array('is_deleted' => 1), $where);
        $db->delete(TABLE_PREFIX . "favorite_links", array(
            'user_id' => $user_id
        ));
        try {
            $sql = "DELETE FROM " . TABLE_PREFIX . "term_relationships WHERE object_type = 'link' AND 
                    object_id IN (SELECT id FROM " . TABLE_PREFIX . "links WHERE user_id=$user_id)";
            $db->exec($sql);
        } catch (Exception $ex) {
        }
    ...
    public function bannedUser($capability, $user_id) {
        return $this->DB()->update(TABLE_PREFIX . "users", array('role' => 'banned', 'capability' => $capability), array('id' => $user_id));
    }
