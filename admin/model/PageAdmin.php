...
class PageAdminModel extends Model {
    ...
    function delete($id) {
        $this->DB()->delete(TABLE_PREFIX . 'pages', "id=" . $id);
    }

    function publish($id) {
        return $this->DB()->update(TABLE_PREFIX . 'pages', array('post_status' => 'published'), "id=" . $id);
    }
    ...
    public function countPages($where = array()) {
        $pages = $this->DB()->select(TABLE_PREFIX . "pages", "COUNT(id) AS total", $where);
