...
class Utils {
    ...
    public static function getPostStatus() {
        $status = array('published' => 'Published', 'pending_approval' => 'Pending Approval', 'draft' => 'Draft', 'trashed' => 'Trashed');
        if (Language::$lang_code == "vi") {
            $status = array('published' => 'Xuất bản', 'pending_approval' => 'Chờ duyệt', 'draft' => 'Nháp', 'trashed' => 'Thùng rác');
        }
        return $status;
    }
    public static function getPostShowInSlider() {
        $status = array('show' => 'Hiển thị', 'hide' => 'Không');
        return $status;
    }
    public static function getCategoryStyle() {
        $status = array('style1' => 'Style 1', 'style2' => 'Style 2', 'style3' => 'Style 3', 'style4' => 'Style 4');
        return $status;
    }

    public static function getRssStatus($num) {
        $status = array(
            0 => '<span class="label label-success">Live</span>',
            1 => '<span class="label label-warning">Die'
        );
        if (Language::$lang_code == "vi") {
            $status = array(
                0 => '<span class="label label-success">Sống</span>',
                1 => '<span class="label label-warning">Chết</span>'
            );
        }
        return $status[$num];
    }

    public static function yesNo($num) {
        if ($num == 1) {
            return '<span class="label label-success">' . Language::$phrases['context']['yes'] . '</span>';
        } else {
            return '<span class="label label-default">' . Language::$phrases['context']['no'] . '</span>';
        }
    }

    public static function getMenuLocation() {
        $status = array('primary_menu' => 'Primary Menu', 'second_menu' => 'Second Menu', 'footer_menu' => 'Footer Menu');
        if (Language::$lang_code == "vi") {
            $status = array('primary_menu' => 'Menu chính', 'second_menu' => 'Menu phụ', 'footer_menu' => 'Menu đối tác');
        }
        return $status;
    }
